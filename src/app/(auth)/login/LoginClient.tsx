...
export default function LoginClient() {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    setTimeout(() => {
      dispatch(
        login({
          name: email.split("@")[0].replace(/[^a-zA-Z]/g, " ") || "Valued Customer",
          email,
        })
      );
      setIsLoading(false);
  ...
  const handleDemoLogin = () => {
    setEmail("vip.client@luxe-jewels.com");
    ...
    setTimeout(() => {
      dispatch(
        login({
          name: "Eleanor Vance",
          email: "vip.client@luxe-jewels.com",
        })
      );
      setIsLoading(false);
      router.push("/");
    ...
    <div className="min-h-[calc(100vh-0px)] bg-[#0f0e0d] text-cream flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg bg-[#161412] border border-gold/25 rounded-2xl shadow-2xl overflow-hidden relative z-10 p-6 sm:p-10 lg:p-12">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-white/10 mb-8">
          ...
          <div className="flex items-center bg-white/5 border border-white/10 rounded-full p-1 text-caption">
        ...
        <div className="mb-8">
          ...
          <p className="text-body text-cream/60 mt-1 font-light leading-relaxed">
            Sign in to view your orders, saved wishlist items &amp; exclusive member rewards.
          </p>
        </div>

        {/* Forgot Password Banner if Clicked */}
        {forgotNotice && (
          <div className="mb-6 p-3.5 rounded-xl bg-gold/10 border border-gold/30 text-caption text-gold flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            <span>A password reset link has been dispatched to your email address.</span>
            <button
              type="button"
              onClick={() => setForgotNotice(false)}
              className="text-gold/80 hover:text-gold font-bold ml-2 cursor-pointer"
            >
              ✕
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center justify-between pt-1">
            <label className="flex items-center gap-2 cursor-pointer group">
              <input
                ...
                className="w-4 h-4 rounded-md border border-white/20 bg-white/5 text-gold focus:ring-0 focus:ring-offset-0 cursor-pointer accent-gold"
              />
              <span className="text-caption text-cream/70 group-hover:text-cream transition-colors">
                Keep me signed in
              </span>
            </label>
          </div>
          ...
          {error && (
            <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-caption text-red-400">
              {error}
            </div>
          )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-white/15 bg-white/5 hover:bg-white/10 text-cream text-caption font-medium transition-colors cursor-pointer"
          >
